...
const els = {
  ...
  setupForm: $("#setupForm"),
  setupStudentName: $("#setupStudentName"),
  setupCurrentScore: $("#setupCurrentScore"),
  setupTargetScore: $("#setupTargetScore"),
  setupDailyQuota: $("#setupDailyQuota"),
  setupTestDate: $("#setupTestDate"),
...
function renderAccountStatus(message = "") {
  ...
  els.setupAccountStatus.textContent = user
      ...
      : configured
        ...
        : "Connect Google to create a trackable student account."
  ...
  if (message) {
    ...
    els.accountStatus.textContent = location.protocol === "file:"
      ...
      : "Firebase is configured. Connect Google to save progress across devices.";
  } else {
...
function renderSettings() {
  ...
  els.setupStudentName.value = state.studentName;
  els.setupCurrentScore.value = state.currentScore;
  els.setupTargetScore.value = state.targetScore;
  els.setupDailyQuota.value = state.dailyQuota;
  els.setupTestDate.value = state.testDate;
}
...
function applyGoalFormValues(source) {
  state.studentName = source.name.value.trim();
  state.currentScore = Number(source.current.value);
  state.targetScore = Number(source.target.value);
  state.dailyQuota = Number(source.quota.value);
  state.testDate = source.testDate.value;
}
...
function bindEvents() {
  ...
  els.setupForm.addEventListener("submit", (event) => {
    ...
    applyGoalFormValues({
      name: els.setupStudentName,
      current: els.setupCurrentScore,
      target: els.setupTargetScore,
      quota: els.setupDailyQuota,
      testDate: els.setupTestDate
    });
    saveState();
    renderAll();
    switchPage(canPractice() ? "dashboard" : "setup");
  });
...
function init() {
  ...
  switchPage(canPractice() ? (location.hash.replace("#", "") || "dashboard") : "setup");
}
